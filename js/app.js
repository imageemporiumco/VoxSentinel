...
// ----------------------------------------------------
// LOCAL STORAGE & REST BACKEND SYNCHRONIZATION
// ----------------------------------------------------
const STORAGE_KEY = "voxsentinel_history";
...
function getLocalHistory() {
    ...
    // Fallback default demo data if nothing stored
    return (typeof VoiceGuardDemoData !== "undefined" && VoiceGuardDemoData.human) ? [
        VoiceGuardDemoData.human,
        VoiceGuardDemoData.synthetic
    ] : [];
}
...
function saveReportToBackend(features, result, segments) {
    const reportItem = {
        id: "anly_" + Math.random().toString(36).substring(2, 9) + "_" + Date.now().toString(36),
    ...
    // 1. Immediately store to browser LocalStorage (Works seamlessly on GitHub Pages)
    const history = getLocalHistory();
    history.unshift(reportItem);
    setLocalHistory(history);
    updateHistoryUI(history);

    // 2. Optionally sync to backend API if running in full-stack Node.js environment
    const payload = {
        clientFeatures: {
            features,
            result,
            filename: reportItem.filename,
            sizeBytes: reportItem.sizeBytes,
            segments
        }
    };
    ...
    .then(res => res.json())
    .catch(() => {
        // Quietly fallback to client-side localStorage on static GitHub Pages
    });
}
...
function loadHistory() {
    ...
    // Optionally probe backend if online
    fetch("/api/history")
    .then(res => res.json())
    .then(resData => {
        if (resData.success && Array.isArray(resData.data) && resData.data.length > 0) {
            updateHistoryUI(resData.data);
            setLocalHistory(resData.data);
        }
    })
    .catch(() => {
        // Running on GitHub Pages or offline - local history is already active
    });
}
...
function updateHistoryUI(history) {
    ...
    history.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td>${item.filename}</td>
            <td>${(item.duration || 0).toFixed(1)}s</td>
...
window.deleteAnalysis = function(id) {
    ...
    fetch(`/api/analysis/${id}`, { method: "DELETE" }).catch(() => {});
};
    ...
    if (report) {
        ...
        activeAnalysisData = {
            ...
            features: {
                ...
                sampleRate: report.sampleRate || 44100,
                avgRMS: report.metrics ? report.metrics.avgRMS : 0.05,
                peakRMS: report.metrics ? report.metrics.peakRMS : 0.1,
                clippingPercentage: report.metrics ? report.metrics.clippingPercentage : 0,
                speechPercentage: report.metrics ? report.metrics.speechPercentage : 85,
                estimatedSNR: report.metrics ? report.metrics.estimatedSNR : 24,
                avgPitch: report.metrics ? report.metrics.avgPitch : 180,
                pitchStd: report.metrics ? report.metrics.pitchStd : 25,
                avgCentroid: report.metrics ? report.metrics.avgCentroid : 2000,
                avgRolloff: report.metrics ? report.metrics.avgRolloff : 3800,
                avgZCR: report.metrics ? report.metrics.avgZCR : 0.08
            },
            result: {
            ...
            pitches: report.pitches || Array.from({length: 100}, () => (report.metrics ? report.metrics.avgPitch : 180)),
            rms: report.rms || Array.from({length: 100}, () => (report.metrics ? report.metrics.avgRMS : 0.05)),
            centroids: report.centroids || Array.from({length: 100}, () => (report.metrics ? report.metrics.avgCentroid : 2000))
        };
        ...
        Charts.drawWaveform(document.getElementById("waveform-canvas"), activeAnalysisData.rms, activeAnalysisData.segments, 0, 1, -1, { duration: report.duration || 5 });
        ...
        ctxSpec.fillText("Historical summary loaded from local forensic registry.", canvasSpec.width / (2 * (window.devicePixelRatio || 1)), canvasSpec.height / (2 * (window.devicePixelRatio || 1)));
...
function runVoiceComparison() {
    ...
    // Instant client-side DSP comparison for GitHub Pages and offline performance
    ...
    .catch(() => {
        // Fallback already rendered locally
    });
}
